<template>
  <div class="bg-gray-50 min-h-screen flex flex-col">
    ...
    <main class="container mx-auto px-4 py-8 flex flex-col md:flex-row">
      <!-- 左侧导航 -->
      <aside class="md:w-64 lg:w-72 flex-shrink-0 mb-6 md:mb-0">
        <div class="md:sticky md:top-8 bg-white rounded-lg shadow p-4 transition-all duration-300 transform hover:shadow-lg">
          <h2 class="text-xl font-semibold mb-4 text-blue-800 border-b pb-2">目录</h2>
          <nav class="space-y-2 max-h-[calc(100vh-200px)] overflow-y-auto pr-2 custom-scrollbar">
            <a 
              v-for="(section, index) in sections" 
              :key="index" 
              :href="`#section-${index + 1}`" 
              :class="[
                'block py-2 px-3 rounded-md transition-all duration-300 flex items-center',
            ...
            >
              <span class="w-6 h-6 flex items-center justify-center bg-blue-600 text-white rounded-full mr-2 text-xs font-medium">{{ index + 1 }}</span>
      ...
      <!-- 条款内容 -->
      <div class="md:flex-1 md:ml-8 bg-white rounded-lg shadow-lg p-6 md:p-8">
        <div class="space-y-8">
          <section 
            v-for="(section, index) in sections" 
            :key="index" 
            :id="`section-${index + 1}`" 
            class="pb-6 border-b border-gray-200 scroll-mt-8"
            ref="sectionRefs"
          >
...
const checkVisibleSection = () => {
  ...
  for (let i = sectionRefs.value.length - 1; i >= 0; i--) {
    ...
    if (section && section.offsetTop <= scrollPosition) {
      activeSection.value = `section-${i + 1}`;
      return;
    }
  }
